...
function reiniciarMicrofono() {
    if (sistemaIniciado && !iaHablando) {
        setTimeout(() => {
            try { recognition.start(); console.log("%c 🎤 Micrófono activo", "color: #34495e;"); } catch(e) {}
        }, 500);
    }
}
...
recognition.onresult = async (event) => {
    ...
    if (text.includes("lee") || text.includes("leer")) {
        responderConVoz("Aquí dice: " + contenidoSlide);
        return;
    }
...
// 4. CEREBRO IA (MODELO ACTUALIZADO)
    ...
    try {
        const response = await fetch(API_URL, {
            ...
            body: JSON.stringify({
                ...
                messages: [
                    {role: "system", content: "Eres José, asistente de la UDB. Responde en 1 frase corta. FORMATO: VOZ: [Respuesta]"},
                    {role: "user", content: `Contexto: ${contexto.substring(0, 400)}. Pregunta: ${frase}`}
                ],
                temperature: 0.6
            })
        });
        const data = await response.json();
        if (data.error) return "VOZ: Error en el modelo, intenta de nuevo.";
